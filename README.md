...
- 单区域历史趋势图
- 1 小时至 30 天统计窗口，并可切换“全部历史”读取项目保留的所有记录
- 原始记录 CSV 导出
- 默认监控 48 小时，也支持不限时运行
- 网页支持暂停、继续和清空历史后重新开始
- 点击表头即可按各指标升序或降序排列
...
- 项目管理支持“导出/导入”项目：把项目连同全部检测记录导出为文件，复制到其他电脑导入后即可统一查看不同网络环境的数据
- 趋势图按本机时间区分白天（06–19 点）、晚上（23–06 点）、高峰（19–23 点）并分别着色
- 界面支持白天/夜间/跟随系统主题，一键太阳月亮图标切换
